...
uint64_t get_dist(pair<int, int>& a, pair<int, int>& b, vector<bool>& is_empty_row, vector<bool>& is_empty_col)
{
	auto [ay, ax] = a;
	auto [by, bx] = b;
	uint64_t res = abs(ay - by) + abs(ax - bx);

	for (int y = min(ay, by) + 1; y < max(ay, by); y++) {
		res += is_empty_row[y] * (1000000ULL - 1);
	}
	for (int x = min(ax, bx) + 1; x < max(ax, bx); x++) {
		res += is_empty_col[x] * (1000000ULL - 1);
	}
