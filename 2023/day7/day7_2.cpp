	...
	void get_score()
	{
		...
		for (int i = 2; i < 15; i++) {
			if (freq[i] == 5) {
				...
				score = 5 + (freq[JOLLY] == 1);
				return;
			} else if (freq[i] == 3) {
				has_three = true;
			} else if (freq[i] == 2) {
				if (has_two) {
					score = freq[JOLLY] == 1 ? 4 : 2;
					return;
				} else {
					has_two = true;
				}
		...
		if (has_three && has_two) {
			...
			score = freq[JOLLY] == 2 ? 6 : freq[JOLLY] == 1 ? 5 : 3;
		} else if (has_two) {
			score = freq[JOLLY] == 3 ? 6 : freq[JOLLY] == 2 ? 5 : freq[JOLLY] == 1 ? 3 : 1;
		} else {
			score = freq[JOLLY] == 3 ? 5 : freq[JOLLY] == 2 ? 3 : freq[JOLLY] == 1 ? 1 : 0;
		}
	}
