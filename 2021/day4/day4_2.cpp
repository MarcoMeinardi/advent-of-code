...
int main () {
	...
	for (i = 0; i < nums.size(); i++) {
		boards.remove_if ([&] (struct board* board) {return board->mark_and_check(nums[i]);});
		if (boards.size() == 1) {
			break;
		}
	}
	...
	for (i++; i < nums.size(); i++) {
		if(boards.front()->mark_and_check(nums[i])) {
			cout << (boards.front()->unmarked_sum * nums[i]) << endl;
			return 0;
		}
	}
