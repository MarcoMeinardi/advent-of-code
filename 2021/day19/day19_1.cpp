...
struct point {
	...
	int64_t compress() {
		return ((int64_t)x << 32)  + ((int64_t)y << 16) + z;
	}
};
...
bool match(vector <point>& a, vector <point>& b, set <int64_t>& points) {
	int matches;
	point inverse;
	for (int r = 0; r < 24; r++) {
...
int main () {
	...
	while(scanf("%[^\n]", foo) == 1) {
		...
		while(scanf("%d,%d,%d", &x, &y, &z) == 3) {
			scanner.back().emplace_back(x, y, z);
		}
	}
	done.resize(scanner.size());
	todo.resize(scanner.size());
	todo[0] = true;
	for (point& p : scanner[0]) {
		points.insert(p.compress());
	}
	...
	do {
		for (size_t i = 0; i < scanner.size(); i++) {
			...
			for (size_t j = 0; j < scanner.size(); j++) {
				...
				if (match(scanner[i], scanner[j], points)) {
					todo[j] = true;
				}
			}
