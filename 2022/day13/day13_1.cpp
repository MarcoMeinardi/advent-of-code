...
struct nested_list_t {
	...
	nested_list_t() : value(-1) {}
	nested_list_t(int val) : value(val) {}
};

shared_ptr <nested_list_t> build_nested_list(const string& repr) {
...
int main () {
	...
	int result = 0;
	while (cin >> line) {
		shared_ptr <nested_list_t> list1 = build_nested_list(line);
		cin >> line;
		shared_ptr <nested_list_t> list2 = build_nested_list(line);
