...
int dfs(int node, int depth = 0) {
	if (depth >= 30 || vis[node]) return 0;
	vis[node] = true;
	int node_flow = flows[node] * (30 - depth - 1);
	if (flows[node]) depth++;  // little cheat because flows["AA"] = 0
							   // otherwise, we need to start from every node with starting depth = costs[0][node]
	
	int res = 0;
	for (int i = 0; i < n; i++) {
		if (flows[i] == 0) continue;
		res = max(res, dfs(i, depth + costs[node][i]));
	}
